<br><p class="lead">PAGE = <?php echo $_GET['page']; ?> </p>
<div id="popup">
	<div class="popup-box">
		<div class="popup-box-content">
			<h1>Não se esqueça!</h1>
			<img style="padding: 5px; width: 30%; height: 30%;" src="in_img/paper-pen.png">
			<p class="lead">Lembre-se de ter em mãos papel e caneta para a resolução dos exercícios.</p>
			<a href="javascript:void(0)" onclick="open_close_popup();" class="btn btn-lg btn-default">Prosseguir</a>
		</div>
	</div>
</div>